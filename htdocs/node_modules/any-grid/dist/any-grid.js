...
AnyGrid.prototype._getItemLayoutPosition = function( item ) {
    ...
    var width = (this.containerWidth / this.columns.length) + ((paddingLeft + paddingRight) / this.columns.length);

    item.element.style.width = width + 'px';

    item.index = this.index;

    var row = Math.floor( this.index / this.columns.length ) + 1;
    item.row = row;
    ...
    if (this.options.stacked) { // need the row for removeVerticalGutters
        column = Math.floor(this.index / this.rowsCount);
        row = Math.abs((column * this.rowsCount) - this.index) + 1;
    }
